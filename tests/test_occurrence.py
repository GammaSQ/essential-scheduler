...
from scheduler.models.occurrences import Occurrence, OccurrenceSubclasses
from scheduler.models.events import ThirdSub
...
class TestOccurrence(TestCase):
    ...
    def test_derived_occurrence_class(self):
        subEv = ThirdSub.objects.create(**self.recurring_data)
        occurrences = subEv.occurrences_after(self.start)
        self.assertTrue(isinstance(next(occurrences), Occurrence))
        self.assertEqual(type(next(occurrences)), OccurrenceSubclasses["ThirdSub"])
